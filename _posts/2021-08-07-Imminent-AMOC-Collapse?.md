...
$$ \frac{\partial x}{\partial t} = \lambda \times x + noise, $$
where dx/dt is the rate-of-change of x with time. The equilibrium is stable if lambda is negative since any perturbation will be dampened by the lambda x term, which restores x back towards zero. 
$a^2$
